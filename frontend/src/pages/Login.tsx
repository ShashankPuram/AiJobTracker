...
export default function Login() {
  ...
  const performLogin = async (currentAttempt = 1, maxAttempts = 2) => {
    try {
      ...
      if (currentAttempt < maxAttempts) {
        setError(`Backend is waking up from sleep (Render free tier). Retrying in 12s...`);
        setTimeout(() => performLogin(currentAttempt + 1, maxAttempts), 12000);
      } else {
        setError("Server is currently unreachable after multiple attempts. Please try again later.");
        setLoading(false);
      }
    }
  };
